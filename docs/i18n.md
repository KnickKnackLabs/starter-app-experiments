...
apps/desktop/.storybook/  # Desktop Storybook
  i18n.ts                 # Storybook i18n initialization
  preview.tsx             # I18nextProvider decorator + RTL support
...
RTL on native uses a **hybrid approach**:

1. **`I18nManager.forceRTL()`** - Sets the native RTL state (requires app reload)
2. **Conditional JS styling** - Handles positioning until reload takes effect

```tsx
// In LanguagePicker, when RTL direction changes:
I18nManager.allowRTL(newIsRtl);
I18nManager.forceRTL(newIsRtl);
// Then prompt user to reload the app

// For positioning that should work immediately:
...
`I18nManager` changes require an app reload to take effect. The implementation:
- Calls `I18nManager.forceRTL()` to set the native state
- Prompts the user to reload the app
- Uses conditional JS styling for elements that need immediate feedback
...
- RTL applied via conditional JS styling
...
**Note:** Unlike the main app, Native Storybook does not use `I18nManager.forceRTL()` or trigger an app reload when switching RTL languages. This is intentional - reloading would disrupt the testing workflow. The `useFlexDirection()` hook in `localization.stories.tsx` compensates by detecting mismatches between the current language and `I18nManager.isRTL` state. Some native components may not flip correctly until the app is restarted.
